...
//
// `Ordinal.\`Protocol\`` is a SIBLING to `Carrier<Ordinal>` (not a
// refinement). Its sole reason to exist is the `associatedtype Count:
// Carrier<Cardinal>` machinery — per-conformer concrete `Count` is
// what makes `slot + .one` infer cleanly at call sites:
//
//   - Ordinal.Count               == Cardinal              (bare)
//   - Tagged<Slot, Ordinal>.Count == Tagged<Slot, Cardinal> (Tagged)
//
// `Carrier<Ordinal>` conformance (in `Ordinal+Carrier.swift`) handles
// the cross-type generic-dispatch role; this protocol handles the
...
extension Ordinal {
    ...
    ///
    /// `Ordinal.\`Protocol\`` is a SIBLING to `Carrier<Ordinal>` — it
    ...
    /// `func describe<C: Carrier>(_ c: C)`) constrain on
    /// `some Carrier<Ordinal>`.
    ///
    /// ## Conformers
    ///
...
extension Tagged: Ordinal.`Protocol` where RawValue: Ordinal.`Protocol`, Tag: ~Copyable {
    ...
    ///
    /// `@_disfavoredOverload` to defer to `Tagged: Carrier`'s
    /// `init(_ underlying: Underlying)` when both apply (same effective
    /// signature for `RawValue == Ordinal`).
    @_disfavoredOverload
    @inlinable
    public init(_ ordinal: Ordinal) {
